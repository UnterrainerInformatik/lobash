...
# ---------------------------------------------------------------------------------------
# _parse_ini_file [--boolean --prefix STRING] file
#
# Parses given ini file using Ruediger Meier's "simple INI file parser".
#
# Example:
# 1: _parse_ini_file mycfg.ini
# 2: _parse_ini_file "mycfg.ini" --prefix "TESTING"
# 3: _parse_ini_file --prefix "I" bashlib.config
#
...
#
# 
# Important: This function expects that `_set_verbose` was called before.
#
...
_parse_ini_file () {
  ...
  if [[ ${rc} != 0 ]] ; then
    _die "_read_ini exited with error code ${rc}."
  fi
